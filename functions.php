...
function check_for_theme_updates($checked_data) {
    ...
    $github_url = 'https://api.github.com/repos/manuseiro/energiasolar/releases/latest';
...
function theme_api_check($false, $action, $response) {
    if ($action === 'theme_information' && isset($response->slug) && $response->slug === 'energiasolar') {
        $github_url = 'https://api.github.com/repos/manuseiro/energiasolar/releases/latest';
        $request_args = array(
            'headers' => array(
                'Accept' => 'application/vnd.github.v3+json',
